...
public class ApiCallUnitTest {
  ...
  public void test_live_server_basic() {
    String bodyAsString =
        RestAssured.get("https://thinder-api.herokuapp.com/helloOpen").body().asString();
...
*/
  ...
  public void attempt_to_registrate_user_with_wrong_password_format() {
    ...
    Result registrationResult =
        UserRepository.getInstance().register("max", "mustermann", "a", "max@kit.edu");
    Assert.assertEquals(registrationResult.getSuccess(), false);

  }

  @Test
  public void attempt_to_registrate_user_with_correct_password_format_and_supervisor_mail()
      throws InterruptedException {
    Result registrationResult =
        UserRepository.getInstance().register("max", "mustermann", "Password123!", "max1@kit.edu");
    Thread.sleep(1000);
  ...
  public void attempt_to_registrate_user_with_correct_password_format_and_student_mail() {
    Result registrationResult = UserRepository.getInstance()
        .register("max", "mustermann", "Password123!", "max@student.kit.edu");
    Assert.assertEquals(registrationResult.getErrorMessage(), null);
