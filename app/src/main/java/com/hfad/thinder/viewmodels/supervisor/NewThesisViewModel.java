...
public class NewThesisViewModel extends ThesisViewModel {
  ...
  public void save() {
    ...
    Thesis thesis =
        new Thesis(getProfessor().getValue(), getTitle().getValue(), getMotivation().getValue(),
            getTask().getValue(), new Form(getQuestions().getValue()), imageSet,
            (Supervisor) UserRepository.getInstance().getUser(), degreeSet);
  ...
  private class SaveTask extends AsyncTask<Thesis, Void, Result> {
    ...
    protected Result doInBackground(Thesis... theses) {
      return thesisRepository.addThesis(theses[0]);
    }
